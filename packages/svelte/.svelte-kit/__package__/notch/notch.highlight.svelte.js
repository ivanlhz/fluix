...
export function createNotchHighlight() {
    ...
    function onItemEnter(e, opts) {
        const target = e.target.closest("a, button");
        const rect = opts.hoverBlobEl;
        const root = opts.rootEl;
        if (!target || !rect || !root || !opts.isOpen)
            return;
    ...
    function resetImmediate(hoverBlobEl, rootW, rootH) {
    ...
    function onItemLeave(hoverBlobEl, springConfig) {
        const rect = hoverBlobEl;
        if (!rect)
            return;
        if (highlightAnim) {
