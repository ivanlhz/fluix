...
export function createNotchHighlight() {
	const hlPrev: HlPrev = { x: 0, y: 0, w: 0, h: 0, visible: false };
	...
	function onItemEnter(
		...
		const rect = ctx.hoverBlobEl;
		const root = ctx.rootEl;
		if (!target || !rect || !root || !ctx.isOpen) return;
		...
		const padX = 8;
		const padY = 4;
		const blobOvershoot = Math.max(6, ctx.roundness * 0.35);
	...
	function resetImmediate(rect: SVGRectElement | null, rootW: number, rootH: number) {
		if (!rect) return;
		if (highlightAnim) {
			highlightAnim.cancel();
			highlightAnim = null;
	...
	function onItemLeave(rect: SVGRectElement | null, springConfig: SpringConfig) {
		if (!rect) return;
		if (highlightAnim) {
			highlightAnim.cancel();
			highlightAnim = null;
	...
	return { onItemEnter, resetImmediate, onItemLeave };
}
